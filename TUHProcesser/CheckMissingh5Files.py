    ...
    for sub in tqdm(sub_ids):
        ...
        for sess in sessions:
            ...
            for mont in monts:
                ...
                for rec in recs:
                    tmp = rec[:-4]+".h5"
                    if not os.path.isfile(os.path.join(data_path, grp,sub,sess, mont,tmp)):
                        lst.append([os.path.join(data_path, grp,sub,sess, mont, rec)])
                    else:
                        cnt += 1
print(cnt)
with open(os.path.join(out_path, 'missingh5.tsv'), 'w', newline='') as tsvfile_recs:
    writer_recs = csv.writer(tsvfile_recs, delimiter='\t', lineterminator='\n')
    writer_recs.writerow(['path'])
