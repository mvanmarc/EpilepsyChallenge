    ...
    for sub in tqdm(sub_ids):
        ...
        for sess in sessions:
            ...
            for mont in monts:
                ...
                for rec in recs:
                    tmp = rec[:-4]+".h5"
                    if os.path.isfile(os.path.join(data_path, grp,sub,sess, mont,tmp)):
                        lst.append([os.path.join(data_path, grp,sub,sess, mont, tmp)])
                    else:
                        cnt += 1
print(cnt)
with open(os.path.join(out_path, 'existingh5.tsv'), 'w', newline='') as tsvfile_recs:
